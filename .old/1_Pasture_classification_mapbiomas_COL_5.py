...
if pasture_type == 0:
    ...
    Tile2Process = tiles
    Col2Process = ee.FeatureCollection(
    ...
    Col2Process = ee.FeatureCollection(
        "users/vieiramesquita/LAPIG-PASTURE/VECTORS/mapbiomas_col3_1_ALL_STAGES_bin_12_03_2018_QGIS"
    )
...
config = {
        ...
        "secondary": [
            {
                ...
                "reducers": [
                    ...
                    {"type": "reducer", "suffix": "stdDev", "obj": ee.Reducer.stdDev()},
                    {
                        "type": "reducer",
                        "suffix": "p10",
                        "obj": ee.Reducer.percentile([10]),
                    },
                    {
                        "type": "reducer",
                        "suffix": "p25",
                        "obj": ee.Reducer.percentile([25]),
                    },
                    {
                        "type": "reducer",
                        "suffix": "p75",
                        "obj": ee.Reducer.percentile([75]),
                    },
                    {
                        "type": "reducer",
                        "suffix": "p90",
                        "obj": ee.Reducer.percentile([90]),
                    },
                    {"type": "amplitude"},
                ],
            }
    ...
    "cloud": {"cloudCoverThreshold": 80, "gapfillValue": -1},
    "trainning": {
        "scale": 30,
        "nPoints": 2700,
        "gridCellSize": 3,
        ...
        "referenceMask": ee.Image("users/lealparente/pasture_mask_v8").expression(
            "(b('b1') == 1) ? 1 : 0"
        ),
    },
    "classification": {
...
def getMetrics(serie, gridCell, gridNumber):
    ...
    gridWRS = ee.FeatureCollection(
        "users/vieiramesquita/LAPIG-PASTURE/VECTORS/LANDSAT_GRID_V2_PASTURE"
    ).filter(ee.Filter.eq("SPRNOME", gridname))

    def clipCollection(img):
        wrsProps = (
            ee.Number(img.get("WRS_PATH"))
            .format()
            .cat("/")
            .cat(ee.Number(img.get("WRS_ROW")).format())
        )
    ...
    def calcPrimaryMetrics(img):
        ...
        for primaryMetric in primaryMetrics:
            ...
            metricImg = img.expression(primaryMetric["expression"]).select(
                [0], [primaryMetric["name"]]
            )
    ...
    metrics = (
        imgCollection.filterMetadata("WRS_PATH", "equals", WRS_PATH)
    ...
    for secondaryMetric in secondaryMetrics:
        metricFilter = metrics.select([secondaryMetric["band"]]).reduce(
            ee.Reducer.percentile([secondaryMetric["percentile"]])
        )
        ...
        for reducer in secondaryMetric["reducers"]:
            ...
            for primaryMetric in primaryMetrics:
                ...
                if reducer["type"] == "reducer":
                    ...
                    bandName = (
                        "amp_" + primaryMetric["name"] + "_" + secondaryMetric["name"]
                    )
                    newBandName = bandName

                    metricImg = metricImg.addBands(
                        metricImg.expression(
                            "b('" + maxBand + "') - b('" + minBand + "')"
                        ).select([0], [bandName])
                    )
    ...
    for tertiaryMetric in tertiaryMetrics:
        tertiaryMetricImg = result.expression(tertiaryMetric["expresion"]).select(
            [0], [tertiaryMetric["name"]]
        )
    ...
    return (
        ee.Image(getLatLong(result))
        .addBands([elevation, slope])
        .set({"BandNumber": bandSize})
    )
...
def getClassifiers(tile, gridCell, neighborsGridCell, serie, neiYears):
    ...
    if year > 2017:
        ...
        for l in range(0, len(seriesY)):
            ...
            neibData = ee.Image(
                ...
                .filter(ee.Filter.gt("BandNumber", 0))
                .mosaic()
            )

            sPoints = Col2Process.filter(ee.Filter.neq(yearlist[l], None)).select(
                [yearlist[l]], ["classValues"]
            )

            def getFeatures(n):
                return n["feature"]

            getNeibArea = ee.FeatureCollection(
                list(map(getFeatures, neighborsGridCell))
            )

            neiTrainningDataset = neibData.sampleRegions(
                collection=sPoints.filterBounds(getNeibArea.geometry()),
            ...
            if neiTrainningDataset is not None:
                ...
                logger.warning(
                    "Classifier %s can't be generate. There weren't enough samples", key
                )
        ...
        neibData = ee.Image(
            ee.ImageCollection(neibData).filter(ee.Filter.gt("BandNumber", 0)).mosaic()
        )

        sPoints = Col2Process.filter(ee.Filter.neq(ColClass, None)).select(
            [ColClass], ["classValues"]
        )
    ...
    try:
        ...
        if neiTrainningDataset is not None:
            trainningDataset = trainningDataset.set(
                "band_order", neiTrainningDataset.get("band_order")
            )

        # .classifier = ee.Classifier.randomForest(nTrees, variablesPerSplit, minLeafPopulation, bagFraction, False, seed);
        classifier = ee.Classifier.smileRandomForest(
            nTrees, variablesPerSplit, minLeafPopulation, bagFraction, None, seed
        )
...
def doClassification(metrics, gridCell, classifiers):
    ...
    finalClassification = finalClassification.set(
        "system:footprint", metrics.get("system:footprint")
    )
...
def checkPoolState(config, taskPool):
    ...
    logger.info(
        "The download's pool is full... waiting %s secs",
        config["download"]["poolCheckTime"],
    )
    ...
    for task in list(taskPool):
        ...
        if taskStatus in (
            ...
            if "error_message" in status and (
                status["error_message"] == "User memory limit exceeded."
                or ("Earth Engine memory capacity exceeded." in status["error_message"])
                or ("Computation timed out" in status["error_message"])
                or ("internal error" in status["error_message"])
                or ("Unable to write to bucket" in status["error_message"])
            ):
                ...
                if reRunCount < config["download"]["attempts"]:
                    ...
                    reRuntask = ee.batch.Export.image(
                        mapResult[taskId]["result"], taskId, mapResult[taskId]["config"]
                    )
    ...
    for pInc in range(int(path - incNumber), int((path + incNumber) + 1)):
        for rInc in range(int(row - incNumber), int((row + incNumber) + 1)):
            ...
            if tAux in tileDict:
                ...
                gridCell = (
                    ee.FeatureCollection(config["grid"]["ftCollection"])
    ...
    while execFlag:
        try:
            ...
            for taskId, serieResult in seriesResult.items():
                ...
                logger.info(
                    "Export classification %s.tif %s", taskId, str(taskConfig["region"])
                )
