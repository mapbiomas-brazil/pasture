...
def createOutputImage(
    referenceFile, outputFile, startRow, xSize, data, imageFormat="GTiff"
):
    ...
    outRaster = driver.Create(
        outputFile, xSize, ySize, 1, referenceBand.DataType, ["COMPRESS=LZW"]
    )
...
def writeData(outputBaseDir, inputFiles, startRow, endRow, outputData):
    ...
    xoff, yoff, outStartRow, outEndRow = getOutputWindow(
        referenceFile, startRow, endRow
    )
    ...
    for i in range(0, len(inputFiles)):
        ...
        outputfile = (
            ...
            + "/"
            + inputFile.replace(
                ".tif", "_tsmedian-" + str(startRow) + "-" + str(endRow) + ".tif"
            )
        )
        print(outputfile)
...
def log(*arg):
    ...
    print(
        "{dateStr} [{identifier}] {msg}".format(
            dateStr=dateStr, identifier=identifier, msg=msg
        )
    )
